...
class BaseEnv(gym.Env):
    ...
    def __init__(self,  environment_graph, condition_logic, reward_locations, render_mode=None, info_dict=defaultdict(int)):

        # It should be posssible to use wrapper for one-hot, so no box and other handling necessary.
        self.n_actions = max([len(i) for i in environment_graph.values()]) # Approach for action spaces, not sure if great
        self.n_states = len(environment_graph) # Assuming nodes are states (different from neuro-nav)
    ...
    def _get_obs(self):
        return self.agent_location # needs to have some other implementation for one hot, I fear
